<template>
  <div class="PublicJson flex align-center justify-center overflow-hidden">
    <!-- 用户头像 -->
    <div v-if="userAvatar" class="micAvatar_img flex align-center justify-center">
      <img :src="userAvatar" @error.once="imgAtError" />
    </div>
    <!-- json容器 -->
    <div v-if="jsonName" class="json_box flex align-center justify-center" :class="'PublicJson_' + jsonName" :id="JsonId" />
  </div>
</template>

<script>
/**
 * @name PublicJson
 * @description json动画组件
 *
 * @prop {String} userAvatar 用户头像
 * @prop {String} jsonName json名称（oss目录名称一致，用作访问图片）
 * @prop {Boolean} loop 是否循环播放
 *
 * @param {String} JsonId 生成唯一json动画挂载id
 * @param {Object} PublicJsonObj 存放json动画实例（不需要响应式，所以没有在data中定义）
 * @function loadJson 加载json动画
 * @function destroyJson 销毁json动画
 *
 * @example
 * <PublicJson jsonName="ch_arcs" :userAvatar="$store.state.user_icon" />
 */
import lottie from 'lottie-web'
export default {
  name: 'PublicJson',
  props: ['userAvatar', 'jsonName', 'loop', 'needPause'],
  data() {
    return {
      // isLoop: false
    }
  },
  computed: {
    JsonId() {
      return `${this.jsonName}_${Date.now()}_${Math.floor(Math.random() * 10000)}`
    }
  },
  watch: {
    jsonName: {
      immediate: true,
      handler(newValue) {
        if (newValue) {
          this.loadJson()
        } else {
          this.destroyJson()
        }
      }
    }
  },
  methods: {
    imgAtError(event) {
      event.currentTarget.src =
        'data:image/png;base64,'
    },
    // 跳转到第1帧并暂停
    // goToAndStop() {
    //   if (this.PublicJsonObj) {
    //     this.PublicJsonObj.goToAndStop(1)
    //   }
    // },
    // 暂停json
    // pauseJson() {
    //   if (this.PublicJsonObj) {
    //     this.PublicJsonObj.pause()
    //   }
    // },
    // 播放json
    // playJson() {
    //   if (this.PublicJsonObj) {
    //     this.PublicJsonObj.play()
    //   }
    // },
    // 监听json加载完成
    // DOMLoadedCallBack() {
    //   this.$emit('DOMLoaded')
    // },
    // 监听帧数 具体帧数可自定义
    // enterFrameCallBack(e) {
    //   if (e.currentTime >= 48) {
    //     this.$emit('jsonComplete')
    //     this.pauseJson()
    //   }
    // },
    // 监听json播放完成
    // completeCallBack() {
    //   this.$emit('complete')
    // },
    loadJson() {
      this.$nextTick(() => {
        this.destroyJson()
        this.PublicJsonObj = this.jsonFn(this.jsonName, `#${this.JsonId}`, this.loop)
        // this.PublicJsonObj.addEventListener('DOMLoaded', this.DOMLoadedCallBack)
        // if (this.needPause) {
        //   this.PublicJsonObj.addEventListener('enterFrame', this.enterFrameCallBack)
        // }
        // if (!this.isLoop) {
        //   this.PublicJsonObj.addEventListener('complete', this.completeCallBack())
        // } else {
        //   this.isLoop = true
        // }
      })
    },
    jsonFn(jsonName, className, loop = true) {
      const params = {
        container: document.querySelector(className),
        renderer: 'svg',
        loop,
        path: `https://imagevo.dandanvoice.com/activity/weekly/json/${jsonName}/data.json`,
        autoplay: true,
        assetsPath: `https://imagevo.dandanvoice.com/activity/weekly/json/${jsonName}/images/`
      }
      return lottie.loadAnimation(params)
    },
    destroyJson() {
      if (this.jsonName && this.PublicJsonObj) {
        // this.PublicJsonObj.removeEventListener('DOMLoaded', this.DOMLoadedCallBack)
        // this.PublicJsonObj.removeEventListener('enterFrame', this.enterFrameCallBack)
        // this.PublicJsonObj.removeEventListener('complete', this.completeCallBack)
        this.PublicJsonObj.destroy()
        this.PublicJsonObj = null
      }
    }
  },
  beforeDestroy() {
    this.destroyJson()
  }
}
</script>

<style scoped lang="scss">
.PublicJson {
  position: relative;
  width: 100%;
  height: 100%;
  .micAvatar_img {
    position: absolute;
    width: 100%;
    height: 100%;
    > img {
      width: 70%;
      height: 70%;
      object-fit: cover;
      border-radius: 50%;
    }
  }
  .json_box {
    width: 100%;
    height: 100%;
  }
}
</style>
