<template>
  <div class="layoutPage">
    ...
    <HalloweenParty
      ...
      @sweetsExchange="sweetsExchange"
      @getTaskAward="getTaskAward"
    />
    <!--神奇南瓜兑奖-->
    <div class="pumpkins_award">
      <div class="title_box">
        <public-json jsonName="ch_wspd" />
        <div class="title_name">{{ title_exchange.text }}</div>
      </div>
      <div class="pumpkins_nums flex_across">
        神奇南瓜<img :src="IconPath('sqng_120_120.png')" />（<span
          :style="{
            color:
              title_exchange.finish_nums < title_exchange.max
                ? '#f93834'
                : '#fff190'
          }"
          >{{
            title_exchange.finish_nums > title_exchange.max
              ? title_exchange.max
              : title_exchange.finish_nums
          }}</span
        >/{{ title_exchange.max }}）
      </div>
      <div
        :class="{ already_buy: title_exchange.has_right == 2 }"
        class="pumpkins_btn flex_across"
        @click="pumpkinExchange"
      >
        {{ title_exchange.has_right == 2 ? '已兑换' : '兑换' }}
      </div>
    </div>
    <!-- 底部文本 -->
    ...
    <theme-dialog
      ...
      :offBtn="dialogMark.includes('preview')"
      :class="dialogMark"
    ...
    >
      ...
      <dialog-congrats
        ...
        :btnText="
          dialogData.award[0].type == 'mic'
            ? '立即装扮'
            : dialogData.award[0].type == 'title'
            ? '立即佩戴'
            : '我知道了'
        "
        @onCongratsBtn="onCongratsBtn"
      >
        <template v-slot="{ type, goods_id }">
          <!-- 称号 -->
          <public-json v-if="type == 'title'" jsonName="ch_wspd" />
          <!-- 头饰 -->
          <public-svga
            v-else-if="goods_id == 3"
            svgaName="ts_jjzy"
            :userAvatar="$store.getters.user_icon"
          />
          <!--ts_ymml-->
          <public-svga
            v-else-if="goods_id == 6"
            svgaName="ts_jjzy"
            :userAvatar="$store.getters.user_icon"
          />
        </template>
      ...
      <!--神奇宝箱预览弹框-->
      <div v-if="dialogMark == 'magic_box'" class="magic_box_preview">
        <div class="magic_box_awards_list">
          <div
            v-for="(item, index) in magic_box_awards"
            :key="index"
            class="magic_box_awards_item"
          >
            <div class="magic_box_award_icon flex_across">
              <img :src="IconPath(item.icon)" />
            </div>
            <div class="magic_box_award_name">{{ item.text }}</div>
          </div>
        </div>
        <div class="magic_box_tip">
          宝箱内的神奇南瓜只可获取3个，之后将不再产出
        </div>
        <div class="i_known_btn magic_box_btn" @click="showDialog = false">
          我知道了
        </div>
      </div>
      <!-- 预览弹框 -->
      <div v-if="dialogMark.includes('preview')" class="preview">
        <DialogPreview
          :info="{
            size: 'small',
            animateType: 'json',
...
import DialogPreview from '@/components/Dialog/DialogPreview.vue'
// 规则说明弹框
import RuleDialog from '../components/RuleDialog.vue'
// 派对有礼组件
import HalloweenParty from '../components/HalloweenParty.vue'
// json
