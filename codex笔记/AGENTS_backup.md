...
## Matt Skills 调用规则
- 新项目先运行 `$setup-matt-pocock-skills`；老项目不跑 setup，除非尚未配置且准备长期使用。
- 默认流程：`$grill-me / $grill-with-docs -> $to-prd -> $implement + $tdd`。
- 无项目文档时用 `$grill-me`；有项目文档、`CONTEXT.md`、ADR 或流程文档时用 `$grill-with-docs`。
- 如果对齐阶段判断任务较大，在 `$to-prd` 后加入 `$to-issues -> $triage`。
- bug、失败、性能问题、结果不符合预期时，切到 `$diagnose`。
- 代码难改、难测、结构混乱时，用 `$improve-codebase-architecture`。
