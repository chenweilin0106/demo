...
- $grill-me/$grill-with-docs、$to-prd、$to-issues、$tdd 必须作为 4 个独立阶段顺序执行。
- 每个阶段结束后必须等待用户明确确认；确认前禁止进入下一阶段或修改代码/文件。
- 修改代码/文件前，用 $grill-me 对齐需求；若需要结合项目文档沉淀方案，改用 $grill-with-docs。
- 需求对齐完成后，用 $to-prd 产出 PRD；仅在任务复杂或需长期追踪时写入实质文档。
- PRD 完成后，用 $to-issues 拆分任务，拆到可独立执行、可独立验证的粒度。
- 任务拆分完成后，用 $tdd 进入正式实现阶段。
- 实现或验证中发现 bug 时，用 $diagnose 分析根因，再用 $tdd 完成修复与回归验证。
