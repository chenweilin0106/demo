...
export const toChatRoom = (room_id) => {
  if (!store.getters.uid || !room_id) return
...
// 跳转会员中心
export const toVip = () => {
  if (!store.getters.uid) return
  const params = { uid: store.getters.uid }
  if (isIOS() || window.navigator.userAgent.indexOf('Safari') === -1) {
    window.webkit.messageHandlers.vip_activity.postMessage(params)
  } else {
    window.external.action('/vip')
  }
}
