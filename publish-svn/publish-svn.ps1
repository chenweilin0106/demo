...
function Invoke-SvnWithSpinner {
  param(
    [string[]]$SvnArgs,
    [string]$ErrorMessage,
    [string]$Activity = 'SVN 提交中'
  )

  $svnCommand = Get-Command svn -ErrorAction SilentlyContinue
  $svnPath = if ($svnCommand -and $svnCommand.CommandType -eq 'Application') { $svnCommand.Source } else { 'svn' }
  ...
  $showSpinner = ($Host.Name -eq 'ConsoleHost')
  $spinnerFrames = @('|', '/', '-', '\')
  $frameIndex = 0
  $startTime = Get-Date
  $maxLineLength = 0
  while (-not $process.HasExited) {
    $process.Refresh()
    if ($showSpinner) {
      $elapsed = Format-Duration ((Get-Date) - $startTime)
      $frame = $spinnerFrames[$frameIndex % $spinnerFrames.Count]
      $line = "$Activity $frame 已用时 $elapsed"
  ...
  $stdoutText = if (Test-Path $stdoutPath) { Get-Content -Raw -Encoding Default $stdoutPath } else { '' }
  $stderrText = if (Test-Path $stderrPath) { Get-Content -Raw -Encoding Default $stderrPath } else { '' }
  if (Test-Path $stdoutPath) { Remove-Item -Force $stdoutPath }
  if (Test-Path $stderrPath) { Remove-Item -Force $stderrPath }
