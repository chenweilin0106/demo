<template>
  <van-popup v-model="showPopup" @closed="clickClose" @touchmove.prevent.stop>
    ...
    >
      <!--标题图片-->
      <img class="title-image" :src="IconPath('title_6.png')" />
...
.van-popup {
  ...
  .popup-content {
    ...
    height: 566px;
    pointer-events: auto;
    background: linear-gradient(0deg, #f5ca79, #ef8a5a);
    padding: 8px;
    border-radius: 26px;
    ...
    .inner-content {
      width: 100%;
      height: 100%;
      background: linear-gradient(0deg, #bc5d45, #ac4634);
      border-radius: 18px;
      overflow: hidden;
    }
  }
  ...
  .title-mic-prompt {
    ...
    .Dialog_Buttons {
      ...
      .left-button {
        color: #fff;
        background: url('@/pages/leanYear/assets/btn_5.png') no-repeat left
          top/100% 100%;
      }
...
.keys_btn {
  ...
  .left-button {
    color: #fff;
    background: url('@/pages/leanYear/assets/btn_6.png') no-repeat left top/100%
      100%;
  }
  .right-button {
    color: #e73d27;
    background: url('@/pages/leanYear/assets/btn_7.png') no-repeat left top/100%
      100%;
  }
}
</style>
