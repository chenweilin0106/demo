...
import { diamondRechargePart } from '@/pages/618/utils/toApp'
...
export default {
  ...
  methods: {
     ...
     */
    confirmBtnCk() {
      getPageData({ type: 'carnival_trader_buy_gift', mark: { gift_id: this.configBuyPopup.gift_list.gift_id, voucher_id: this.configBuyPopup.voucher_list[this.activeVoucher].id } }).then((res) => {
        if (res.errno == 0) {
...
.voucherArea{
  ...
  .voucherTitle{
    ...
    .dropDpwnMark{
      ...
      pointer-events: none;
      &.upStatus{
        background: url('@/pages/618/assets/tk_19.png') no-repeat left top/100% 100%;
      }
      &.downStatus{
        background: url('@/pages/618/assets/tk_20.png') no-repeat left top/100% 100%;
      }
    }
    .voucherListArea{
      width: 335px;
      ...
      .voucherListItem{
        ...
        .activeMark{
          width: 45px;
          height: 36px;
          background: url('@/pages/618/assets/tk_21.png') no-repeat left top/100% 100%;
          position: absolute;
          right: 20px;
          bottom: 10px;
        }
