    ...
    <div class="psBg">
      ...
      <div class="nodeItem" :style="`width: ${100/tot_list.length}%`" v-for="(item) in tot_list" :key="`${item.id}${item.awardId}`">
        ...
        <div :class="['rewardDiv',`status${item.has_right}`]" @click="nodeIconCk(item)">
          ...
          <div class="priceMark" v-if="item.award.show_price">
            {{item.award.show_price}}
            <img :src="IconPath('zs_27_20.png')" />
          </div>
          <!-- <div class="previewTip" v-if="item.previewImg"></div> -->
          <div class="pstMark" v-if="item.award.type=='pst_privilege'">赠送权</div>
          <div class="tumeMark" v-if="item.award.type=='costume'">主页特效</div>
          <!-- <van-popover
            v-model="item.showPopover"
            v-if="item.awardId==36||item.awardId==72"
            trigger="click"
            :placement="item.awardId==36?'top':'top-end'"
            theme="dark"
            :offset="item.awardId==36?[+60,30]:[86,30]"
          >
            <div class="popover_span">
              <p v-show="item.awardId==36">有机会获得绿色、蓝色、</p>
              <p v-show="item.awardId==36">紫色、橙色任一品质符石</p>
              <p v-show="item.awardId==72">基础加成：家族战内战力提升10%</p>
              <p v-show="item.awardId==72">额外加成：当CP同时拥有此称号</p>
              <p v-show="item.awardId==72">时，家族战内战力额外提升5%</p>
            </div>
            <template #reference>
...
export default {
  name: 'stepProgress',
  props: ['tot_num', 'tot_list', 'select_date'],
  components: {},
  data() {
    return {
      psPercent: 0, // 进度条百分比
  ...
  methods: {
    ...
    nodeIconCk(item) {
      console.log(item)
      if (item.has_right == 1) {
        getPageData({ type: 'sh_treasure_tot_award', mark: { id: item.awardId, date: this.select_date } }).then((res) => {
          if (res.errno == 0) {
            // 打开获奖弹窗
            this.$emit('openReceiveAwardsPopup', res.data, false)
            this.scrollHas()
    ...
    .nodeItem{
      ...
      .rewardDiv{
        ...
        &.status1::after{
          ...
          width: 144px;
          height: 157px;
          background: url('@/pages/goldenAutumn/assets/mk4_28.png') no-repeat left top/100% 100%;
          position: absolute;
          left: -8px;
          top: -8px;
          z-index: 1;
        ...
        .previewTip{
          width: 118px;
          height: 26px;
          background: url('@/pages/goldenAutumn/assets/mk5_20.png') no-repeat left top/100% 100%;
          position: absolute;
          left: 50%;
          transform: translateX(-50%);
          top: 70px;
