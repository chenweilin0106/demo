<template>
  <div class="TaskListItem margin-row-center">
    <div class="TaskListItem_Main">
      <!-- 两个奖励 -->
      <div v-if="isTwoPresent" class="TaskListItem_TextBox flex_across">
        {{ taskItem.task_name }}
        <span v-if="taskItem.task_name == '每充值10元'">系统自动派发，无需手动领取</span>
        <span class="speical_color flex_across num_box" v-else>（{{ taskItem.finish_times }}/{{ taskItem.max_times }}）</span>
      </div>
      <!-- 任务奖励 -->
      <div class="TaskListItem_IconBox theme_item">
        ...
        <div class="img-bottom TaskListItem_BottomText">
          <span class="flex_across">{{ pointName }}+{{ taskItem.award_nums }}</span>
        </div>
      </div>
      <!-- 闯关币 -->
      <div class="TaskListItem_IconBox theme_item" v-if="isTwoPresent && taskItem.piece">
        <div class="img-top TaskListItem_TopIcon">
...
.TaskListItem {
  ...
  margin-bottom: 19px;
  //border-radius: 20px;
  background: url('@/pages/crazyEgg/assets/bg_1.png') no-repeat left top/100% 100%;
  // 两个奖励
  // padding: 39px 0 6px;
  // 单个奖励
  padding: 3px;
  ...
  .TaskListItem_Main {
    ...
    height: 131px;
    padding-left: 14px;
    //background: linear-gradient(90deg, #848dff, #cfafff);
    //border-radius: 17px;
    // 两个奖励
