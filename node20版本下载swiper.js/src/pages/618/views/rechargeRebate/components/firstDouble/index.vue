<template>
  <OutBox class="FirstDouble" title="title_2.png" :bottom="true">
    ...
    <PublicButton
      ...
      :hasRight="fistDoubleData.has_right"
...
.FirstDouble {
  ...
  .RechargeRebate_TopBox_Body {
    ...
    .RechargeRebate_TopBox_Body_Item {
      ...
      height: 168px;
      padding: 4px;
      background-color: #ebc95b; // 边框 需要更新
      border-radius: 16px;
      .img-box {
        height: 112px;
        background-image: linear-gradient(180deg, #ffe08e, #ffffff); // 背景色 需要更新
      ...
      // 随机奖励样式
      &:nth-child(3) {
        background-color: #ea5927; // 边框 需要更新
        .img-box {
          background-image: linear-gradient(180deg, #fcf2c3, #ffffff); // 背景色 需要更新
        }
        .img-bottom {
          // 背景色 需要更新
  ...
  .RechargeRebate_TopBox_Button {
    ...
    color: #fff;
    &.button-noFinish {
      background-image: linear-gradient(0deg, #87a9e7, #a3dffe); // 未完成状态 需要更新
    }
    &.button-receive {
      background-image: linear-gradient(0deg, #ff853b, #f9d853); // 领取状态 需要更新
      color: #fff;
    }
  }
  .RechargeRebate_TopBox_Article {
    ...
    width: 660px;
    height: 320px;
    background: linear-gradient(0deg, #dfb65d, #d79c48, #b67a4e); // 首充充值文本背景色 需要更新
    border-radius: 30px;
    .articleTitle {
      position: relative;
      margin: 0 auto 20px;
