<template>
  <PopupBox title="title_7.png" @clickClose="clickClose">
    <p v-if="buildAwardsPopupData.specialLayer" class="topTip flex align-center justify-center line-height-100">
      成功冠名<span>{{ buildAwardsPopupData.specialLayer }}</span
      >层
    </p>
    <div class="list flex align-center justify-center flex-wrap">
      <div v-for="(item, index) in buildAwardsPopupData.list" :key="index" class="award flex-column align-center">
 ...
 */
import PopupBox from '@/pages/618/components/popupBox'
import { nowEquip } from '@/pages/618/api'

export default {
  props: ['buildAwardsPopupData'],
  components: { PopupBox },
  data() {
    return {
      // list: [
  ...
  computed: {
    list() {
      return this.buildAwardsPopupData.list
    },
    // 是否装扮
    isEquip() {
