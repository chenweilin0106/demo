<template>
  <van-popup v-model="showPopup" class="flex justify-center align-center" @closed="clickClose" @touchmove.prevent.stop>
    <div class="popup-content" @touchmove.prevent.stop>
      ...
      <!--额外装饰-->
      <img :src="IconPath('back_3.png')" class="leftBalloon position-absolute pointer-none" />
      <img :src="IconPath('back_4.png')" class="rightBalloon position-absolute pointer-none" />
      ...
      <div class="inner w-100 h-100 overflow-y-hidden overflow-x-hidden">
        ...
        >
          ...
          <template v-else-if="dialogData.goods_type === 'title' || dialogData.goods_type === 'mic'">
            <!-- 动画容器 -->
            <public-json v-if="dialogData.goods_type === 'title'" jsonName="ch_ymcf" />
            <public-svga v-if="dialogData.goods_type === 'mic'" svgaName="ts_yycg" :userAvatar="$store.state.user_icon" />
          </template>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      dialogData: {
        ...
        // price_text: '300',
        // max_times: 1,
        // goods_icon: 'cnll_120_120.png',
        // goods_icon_t: 'cnll_120_120.png',
  ...
  computed: {
    ...
    dialogGoodsName() {
      ...
      if (['mic', 'car', 'costume'].includes(goods_type)) return `${goods_name}+${goods_num}天`
      return goods_name
    },
    // 召唤石
    isHeroStone() {
