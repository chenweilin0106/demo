<template>
  <out-box class="GiftBack" bowKnot="true">
    ...
    <CongratsGetPopup v-if="showCongratsGetPopup" :list="congratsGetPopupData" @close="showCongratsGetPopup = false">
      <template v-slot="{ data: { type } }">
        <public-svga v-if="type == 'mic'" svgaName="ts_yycg" :userAvatar="$store.state.user_icon" />
      </template>
    </CongratsGetPopup>
...
import PreviewPopup from '@/pages/gardenParty/components/PreviewPopup.vue'
...
export default {
  ...
  components: { PublicSvga, CongratsGetPopup, PreviewPopup },
  methods: {
...
.awardCard {
  ...
  .awardCard-iconBox {
    ...
    .awardCard-textContent {
      ...
      color: #ffffff;
      background: #c16d6f;
      border-radius: 0px 0px 12px 12px; // GoodsCard-iconBox的border-radius - border-width
    }
  }
  .awardCard-textBox {
    margin: auto 0;
