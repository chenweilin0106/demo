<template>
  <PopupBox title="title_8.png" @clickClose="clickClose">
    ...
    <div class="promptIcon flex align-center justify-center" :class="[dialogData.type === 'title' && 'title', dialogData.goods_type === 'car']">
      <PublicImg :class="dialogData.goods_type" :imgName="dialogData.goods_icon" :imgType="dialogData.goods_type" />
    </div>
...
import popupCount from '@/pages/childrenDay/views/pointReward/components/popupCount.vue'
import { getPageData } from '../../../api'
import { _throttle } from '../../../utils/tool'

export default {
  components: { popupCount, PopupBox },
  data() {
    return {
  ...
  computed: {
    // 绿晶石 紫晶石 科举卡 兑换按钮
    isNormalExchange() {
      return ['keju'].includes(this.dialogData.type) || this.dialogData.type?.startsWith('tools_')
    },
    // 称号 头饰 座驾 声波 聊天气泡 主页特效 兑换按钮
    isSeniorExchange() {
...
.buttonStyle-2 {
  color: #fff;
  background: linear-gradient(0deg, #a42e76, #cf7f8b);
  z-index: 1;
  position: relative;
  &::before {
    content: '';
    ...
    position: absolute;
    inset: 2px;
    background-image: linear-gradient(180deg, #ffd695, #eb63a1);
    border-radius: inherit;
  }
}
// 绿晶石 紫晶石 科举卡 兑换按钮
