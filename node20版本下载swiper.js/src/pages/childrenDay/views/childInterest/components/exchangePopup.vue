<template>
  <PopupBox title="title_15.png" @clickClose="clickClose">
    <div class="ownedNumText">剩余{{pointName}}<img :src="IconPath(pointIcon)" />:&ensp;{{my_goods}}</div>
...
<script>
import { _throttle } from '@/pages/childrenDay/utils/tool.js'
import { getPageData } from '@/pages/childrenDay/api/index'
...
export default {
  ...
  methods: {
    ...
    plusCk() {
      if (this.exchangeNum < (Number(this.config.max_times) - Number(this.config.exchange_times)) || this.config.max_times == -1) {
    ...
    exchangeBtnCk: _throttle(function () {
      getPageData({ type: 'naughty_convert', mark: { id: this.config.goods_id, nums: this.exchangeNum } }).then((res) => {
        if (res.errno == 0) {
          this.clickClose()
