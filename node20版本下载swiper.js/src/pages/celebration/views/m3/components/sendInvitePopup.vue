...
export default {
  ...
  methods: {
    toUserMain,
    getPageData() {
      getPageData({ type: 'ps_invite_list' }).then((res) => {
        if (res.errno == 0) {
          console.log(res)
    ...
    searchBtnCk() {
      ...
      if (this.callCode && this.callCode.length && this.callCode.length > 0) {
        console.log('search')
        getPageData({ type: 'ps_invite_search', mark: this.callCode }).then((res) => {
          if (res.errno == 0) {
            console.log(res)
    ...
    sendInviteCk() {
      if (this.friendsChosenList.length > 0) {
        getPageData({ type: 'ps_invite', mark: this.friendsChosenList }).then((res) => {
          if (res.errno == 0) {
            this.$toast('邀请成功')
            this.clickClose()
          } else {
...
.friendsArea{
  ...
  .friendsShowItem{
    ...
    .userBtn{
      ...
      transform: translateY(-50%);
      &.yesChoose{
        background: url('@/pages/celebration/assets/tk_41.png') no-repeat left top/100% 100%;
      }
      &.noChoose{
        background: url('@/pages/celebration/assets/tk_42.png') no-repeat left top/100% 100%;
      }
    }
  }
}
