  ...
  <div class="component">
    ...
    <div :class="['wheelDiscArea']">
      <div :class="['rewardItem', `rare${item.rare}`]" v-for="(item,index) in rewardsList" :key="`${index}+${item.id}`">
        ...
        <div class="spMark" v-if="item.id==6&&pretty_num!=10">获得{{pretty_num}}/10</div>
        <div class="rewardNone" v-show="item.id==6&&pretty_num==10"></div>
...
export default {
  ...
  data() {
    return {
      rewardsList: [
        { id: 1, rare: 1, active: false, type: 'tool', text: '+100', icon: 'fsjc_120_120.png' },
        { id: 2, rare: 2, active: false, type: 'mic', text: '头饰+1天', icon: 'azsh_120_120.png' },
        { id: 3, rare: 2, active: false, type: 'tool', text: '戒指+1', icon: 'yyjz_120_120.png', mark: '13140' },
        { id: 4, rare: 2, active: false, type: 'qipao', text: '公屏气泡+1天', icon: 'lcyx_180_120.png', is_new: true },
        { id: 5, rare: 2, active: false, type: 'car', text: '超级礼物道具+1', icon: 'wndc_120_120.png' },
        ...
        { id: 8, rare: 1, active: false, type: 'tool', text: '+5', icon: 'zjs_120_120.png' }
      ], // 轮盘数据
    ...
    lotteryBtnCk: _throttle(function(lotteryType) {
      ...
      if (!this.isWheeling) {
        if (Number(this.leftDrawsNum) >= lotteryType) {
          getPageData({ type: 'ps_pray', mark: lotteryType }).then((res) => {
            if (res.errno == 0) {
...
.component {
  ...
  .wheelDiscArea {
    ...
    .rewardItem {
      ...
      position: absolute;
      &.rare1{
        background: url('@/pages/celebration/assets/mk8_15.png') no-repeat left top/100% 100%;
      }
      &.rare2{
        background: url('@/pages/celebration/assets/mk8_14.png') no-repeat left top/100% 100%;
      }
      &:nth-of-type(1) {
        left: 40px;
        top: 100px;
      ...
      .rewardNone{
        ...
        width: 100%;
        height: 100%;
        background: url('@/pages/celebration/assets/mk8_12.png') no-repeat left top/100% 100%;
      }
      .rewardActive::after {
        content: '';
        position: absolute;
        ...
        width: 100%;
        height: 100%;
        background: url('@/pages/celebration/assets/mk8_13.png') no-repeat left top/100% 100%;
      }
    }
  }
  .lotteryArea {
    ...
    .lotteryBtnDiv{
      ...
      .lotteryOne{
        width: 242px;
        height: 128px;
        background: url('@/pages/celebration/assets/mk8_22.png') no-repeat left top/100% 100%;
      }
      .lotteryTen{
        width: 242px;
        height: 128px;
        background: url('@/pages/celebration/assets/mk8_23.png') no-repeat left top/100% 100%;
      }
      .lotteryFifty{
        width: 242px;
        height: 128px;
        background: url('@/pages/celebration/assets/mk8_24.png') no-repeat left top/100% 100%;
      }
    }
  }
}
