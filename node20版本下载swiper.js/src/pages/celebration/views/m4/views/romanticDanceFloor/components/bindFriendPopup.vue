<template>
  <PopupBox title="mk9_title_13.png" @clickClose="clickClose">
    <!--顶部提示-->
    ...
    <!--输入框-->
    <input v-if="step == 1 || step == 2" v-model="uid" :readonly="step == 2" class="bindInput margin-row-center text-center line-height-100 text-nowrap" placeholder="点击输入对方ID" maxlength="8" @keydown="handleKeyDown" @input="inputUid" />
...
export default {
  ...
  methods: {
     ...
     */
    getPopupPage() {
      getPageData({ type: 'tab' }).then((res) => {
        if (res.errno == 0) {
          console.log(res)
          this.step = res.data.status
          if (this.step == 3 || this.step == 4) this.searchResult = res.data.pair_user_info
        } else {
...
.avatarBox {
  ...
  background: #ffffff;
  border-radius: 50%;
  border: 4px solid #A7DFFF; /*todo 头像边框背景色*/
}
.nickName {
  margin-top: 18px;
  width: 177px;
...
.bindInput {
  ...
  background: #fefaef;
  border-radius: 41px;
  border: 4px solid #A7DFFF; /*todo 输入框边框*/
  font-weight: bold;
  font-size: 34px;
  color: #486CEA; /*todo 输入内容颜色*/
  &::placeholder {
    font-weight: bold;
    font-size: 34px;
    color: #c2c4d6;
...
.userInfo {
  ...
  width: 544px;
  height: 252px;
  background: rgba(102, 51, 219, 0.3); /*todo 搜索结果背景色*/
  border-radius: 20px;
  .avatarAndNickName {
    width: 177px;
    height: 100%;
