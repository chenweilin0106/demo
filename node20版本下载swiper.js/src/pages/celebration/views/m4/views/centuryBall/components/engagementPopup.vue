<template>
  <PopupBox :title="'mk9_title_4.png'" @clickClose="clickClose">
    ...
    <div class="area1">
      ...
      <div :class="['btn', `status${buyStatus}`]" @click="buyBtnCk">
        <template v-if="buyStatus == 1">520<img :src="IconPath('zs_42_32.png')" />购买</template>
        <template v-else-if="buyStatus == 2">已购买</template>
        <template v-else-if="buyStatus == 3">已结束</template>
      </div>
    </div>
    <div class="area2">
      <div class="img3"></div>
    ...
    <div class="area3">
      ...
      <div class="img5"></div>
      <div :class="['headDiv', 'rightHead']" @click="clickInvite">
        <img :class="[pair_status?null:'smallSp']" :src="IconPath(pair_status?pair_info[1].profile_image:'tk_56.png')" />
        <div class="userNameDiv"><span>{{pair_status?pair_info[1].username:'邀请好友绑定'}}</span></div>
      </div>
    </div>
  </PopupBox>
</template>
...
.area1{
  ...
  .img1{
    width: 487px;
    height: 33px;
    background: url('@/pages/celebration/assets/tk_52.png') no-repeat left top/100% 100%;
...
.area2{
  ...
  .img3{
    width: 487px;
    height: 33px;
    background: url('@/pages/celebration/assets/tk_53.png') no-repeat left top/100% 100%;
