...
export default {
  ...
  methods: {
    ...
    nodeIconCk(item) {
      console.log(item)
      if (item.has_right == 1) {
        getPageData({ type: 'carnival_trader_rev_tot_consume', mark: item.id }).then((res) => {
          if (res.errno == 0) {
            // 打开获奖弹窗
    ...
    .nodeItem{
      ...
      .rewardDiv{
        ...
        .previewTip{
          width: 118px;
          height: 26px;
          background: url('@/pages/celebration/assets/mk5_20.png') no-repeat left top/100% 100%;
          position: absolute;
          left: 50%;
          transform: translateX(-50%);
          top: 70px;
        ...
        .aslc_leftItem_awardIcon{
          width: 32px;
          height: 32px;
          background: url('@/pages/celebration/assets/mk4_19.png') no-repeat left top/100% 100%;
          position: absolute;
          right: 0;
          top: -10px;
          z-index: 2;
