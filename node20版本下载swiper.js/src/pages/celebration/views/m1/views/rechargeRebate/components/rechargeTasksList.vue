  ...
  <transition-group class="rechargeTasksList flex-column align-center" :name="transitionClass" tag="ul">
    ...
    >
      <div class="RechargeTaskItem_LeftContent">
        ...
        <!-- 奖励渲染 -->
        <p v-if="!isSpecial(rechargeAward)" class="award_contain flex align-center line-height-100 width-fit flex-no-wrap">
          <template v-for="(item, index) in rechargeAward.awards">
            ...
            <template v-if="item.text_add">
              <i :key="index + 10" class="update-icon"></i><span :key="index + 100" class="update-text"> {{ item.text_add }}<i>，</i></span>
            </template>
          </template>
        </p>
        <!-- 特殊奖励列表渲染 -->
...
.rechargeTasksList {
  padding-top: 20px; /*todo 距离tab的距离*/
  .RechargeTaskItem {
  position: relative;
  width: 660px; /*todo 卡片宽度 不需要加上外圈描边宽度*/
  height: 136px; /*todo 卡片高度 不需要加上外圈描边宽度*/
  margin-bottom: 20px; /*todo 卡片底部间距*/
  margin-left: 20px;
  display: flex;
  align-items: center;
    ...
    .PresentList {
      ...
      .PresentItem {
        ...
        .img-box {
          ...
          border-radius: 16px;
          padding: 4px;
          background-color: #DBC1FF; /*todo 奖励边框颜色*/
          .img-top {
            width: 100%;
            height: 90px;
          ...
          .img-bottom {
            ...
            background-color: #7954E4; /*todo 奖励文字背景色*/
          }
        }
        .update-text-2 {
          position: absolute;
