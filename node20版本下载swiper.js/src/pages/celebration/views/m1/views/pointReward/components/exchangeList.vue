<template>
  <OutBox class="exchangeList" title="mk1_title_1.png">
    <!-- 我的道具 -->
    ...
    <PublicCard v-for="goods in goods_list" :key="goods.goods_name" class="exchange-item">
      ...
      />
      <!--右上角特惠角标-->
      <img v-if="goods.goods_name == '藏宝图' || goods.goods_name == '永久称号'" :src="IconPath('mk1_4.png')" class="discountLabel position-absolute" />
    </PublicCard>
    <!-- 规则说明 -->
...
.exchangeList {
  ...
  .exchange-item {
    position: relative;
    display: flex;
    margin: 0 0 19px 43px; /*todo 卡片下方间距*/
    width: 664px; /*todo 卡片宽度 不需要加上外圈描边宽度*/
    height: 164px; /*todo 卡片高度 不需要加上外圈描边宽度*/
    ...
    .left-content {
      height: 100%;
      width: 240px;
      //background: url('@/pages/celebration/assets/') no-repeat left top/100% 100%; // 背景
      .icon-content {
        position: relative;
        display: flex;
        align-items: center;
        ...
        width: 100%;
        height: 124px;
        background: linear-gradient(180deg, #EECEFF, #FFFFFF); /*todo 兑换列表中道具图标背景色*/
