<template>
  <div class="RechargeTaskItem" :class="{ RechargeLevel5000: isLevel5000 }">
    <div class="RechargeTaskItem_LeftContent">
      ...
      <!-- 连续充值 -->
      <p v-else>
        <span>累计{{ itemValue.days_cond }}天</span>每天充值满{{ itemValue.amount_cond }}元<span
          >（{{ itemValue.reach_days }}/{{ itemValue.days_cond }}）</span
        >
      </p>
      <!-- 奖励渲染 -->
      <p v-if="!isLevel5000" class="award_contain flex align-center justify-center">
        <template v-for="(item, index) in itemValue.awards">
          <span :key="item.type" :class="{ goldCard: item.name == '24小时金币卡' }">
            <img :class="ButtonStyle(iconStyle, item.type)" :src="IconPath(item.icon)" alt=""
          /></span>
      ...
      <div v-else class="PresentList">
        <template v-for="item in itemValue.awards">
          <div :key="item.name" v-if="!(item.name === '威望值')" :class="['PresentItem', { 'gift-item_title': item.type === 'title' }]">
            ...
            <div class="text-box flex align-center justify-center">+{{ item.text }}</div>
          </div>
        </template>
      </div>
    </div>
    ...
    <div
      ...
      :class="['flex align-center justify-center', ...buttonStyle]"
      @click="getAward(itemValue.has_right, itemValue.condition || itemValue.amount_cond)"
    >
...
export default {
  ...
  methods: {
    // 点击按钮
    getAward: _throttle(function (flag, id) {
      if (!flag) {
        // 0 去充值 跳转充值界面
...
.RechargeTaskItem {
  ...
  margin-bottom: 20px;
  background: url('@/pages/qingMing/assets/mk2_5.png') no-repeat left top/100% 100%;
  ...
  .special-button {
    ...
    background: url('@/pages/qingMing/assets/btn_5.png') no-repeat left top/100% 100% !important;
    font-weight: bold;
    font-size: 32px;
    color: #f5ffd7;
  }
  ...
  .special-button-1 {
    color: #16474a;
    background: url('@/pages/qingMing/assets/btn_4.png') no-repeat left top/100% 100% !important;
  }

  .special-button-2 {
    color: #464642;
    background: url('@/pages/qingMing/assets/btn_6.png') no-repeat left top/100% 100% !important;
  }
...
.RechargeLevel5000 {
  ...
  height: 368px;
  background: url('@/pages/qingMing/assets/mk2_6.png') no-repeat left top/100% 100%;
