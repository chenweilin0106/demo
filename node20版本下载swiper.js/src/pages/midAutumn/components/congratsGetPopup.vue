<template>
  <PopupBox title="title_10.png" size="2" @clickClose="clickClose">
    <div class="listMain flex align-center justify-center flex-wrap">
      <div v-for="(item, index) in list" :key="index" class="award flex-column align-center" :class="(item.tool_id == 5169 || item.tool_id == 125 || item.tool_id == 4770 || item.tool_id == 4878 || item.tool_id == 4987 || item.tool_id == 4988) && 'large'">
        <div class="icon w-100 position-relative" >
          <!--有些道具无法使用type判断 所以单独写死-->
          <div v-if="typeToNameMap[item.type] || item.id == 226" class="typeLabel">
            {{ item.id == 226 ? '超级道具礼物' : typeToNameMap[item.type] }}
          </div>
...
.listMain {
  ...
  .award {
    ...
    .icon {
      ...
      background-color: #E06224; /*todo 边框颜色*/
      padding: 4px;
      .iconMain {
        width: 100%;
        height: 100%;
