<template>
  <OutBox class="CumulativeSpendingList position-relative" title="title_4.png">
...
.CumulativeSpendingList {
  margin-top: -20px; /*todo 顶部margin 距离tab间距*/
  padding-top: 85px; /*todo 顶部padding 第一个卡片和容器顶部距离*/
  margin-bottom: 0; /*todo 底部margin 和排行榜容器距离 如果没有排行榜模块 则设置为0*/
  .resetIcon {
    z-index: 1;
    top: -58px; /*todo 每日重置图标top 根据容器定位*/
    right: 163px; /*todo 每日重置图标right 根据容器定位*/
    width: 124px; /*todo 宽度*/
    height: 42px; /*todo 高度*/
  }
  // 累积消费返利列表
  .listMain {
    ...
    .taskItem {
      ...
      height: 134px; /*todo 卡片高度 不需要加上外圈描边宽度*/
      border-radius: 12px; /*todo 卡片圆角*/
      .focus-color {
        color: #fffaa6 !important; /*todo xxx钻或者（0/xxx）完成情况*/
      }
      .awardItem {
        $radius: 16px; /*todo 奖励边框圆角*/
        $borderWidth: 4px; /*todo 奖励边框宽度*/
        display: flex;
        flex-direction: column;
        width: 88px; /*todo 奖励图标容器宽度*/
        height: 88px; /*todo 奖励图标容器高度*/
        background-color: #FBE590; /*todo 奖励边框颜色*/
        padding: $borderWidth;
        border-radius: $radius;
        margin-left: 5px; /*todo 奖励左间隔*/
        .awardIcon {
          width: 100%;
          height: 54px;
          background: #fff; /*todo 奖励图标背景色*/
          border-radius: $radius - $borderWidth $radius - $borderWidth 0 0;
          > img {
            width: 85%;
          }
        }
        .awardText {
          ...
          color: #fff;
          width: 100%;
          background-color: #466BD5; /*todo 奖励文本背景色*/
          border-radius: 0 0 $radius - $borderWidth $radius - $borderWidth;
        }
      }
    }
    .taskText {
      margin-left: 16px; /*todo 累计消费...文本和左边奖励的距离*/
      font-size: 24px;
      line-height: 100%;
      p:nth-child(1) {
        font-size: 29px;
