<template>
  <PopupBox :title="'title_9.png'" :size="'small'" @clickClose="clickClose" :hasLight="isHasLight">
    <div class="rewardsListDiv">
      <div :class="['rewardItem', item.type == 'title' ? 'title' : 'other']" v-for="(item, index) in config" :key="index">
        ...
        <div class="markIcon" v-if="item.type=='hammer'&&item.nums>=21">好运降临</div>
...
export default {
  ...
  created() {
    for (let item of this.config) {
      if (item.type == 'hammer' && item.nums >= 21) {
        this.isHasLight = true
        break
      }
    }
  },
