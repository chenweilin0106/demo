<template>
  <OutBox class="FirstDouble" title="title_2.png" :bottom="true">
...
import FirstDoubleAwardPopup from './fistDoubleAwardPopup.vue'
import FirstDoubleRulesPopup from './firstDoubleRulesPopup.vue'
export default {
  name: 'FirstDouble',
...
.FirstDouble {
  ...
  .ruleIcon {
    top: 38px; /*todo 规则图标距离顶部距离*/
    right: 28px; /*todo 规则图标距离右侧距离*/
  ...
  .RechargeRebate_TopBox_Body {
    .RechargeRebate_TopBox_Body_Item {
      ...
      height: 168px;
      padding: 4px;
      background-color: #41ACFF; /*todo 奖励边框颜色*/
      border-radius: 16px;
      .img-box {
        height: 112px;
      ...
      .img-bottom {
        background: #466BD5; /*todo 奖励文字背景色*/
        border-radius: 0 0 12px 12px;
        font-size: 25px;
        color: #fff;
      }
      &:nth-child(2) {
        margin: 0 40px; /*todo 中间奖励距离两边间距*/
      }
      // 随机奖励
      &:nth-child(3) {
  ...
  .RechargeRebate_TopBox_Button {
    ...
    font-size: 32px;
    &.button-noFinish {
      background-image: linear-gradient(0deg, #CAE7FF, #FFFFFF); /*todo 未完成状态按钮背景色*/
    }
    &.button-receive {
      background-image: linear-gradient(0deg, #ff853b, #f9d853); /*todo 领取状态按钮背景色*/
      color: #fff; /*todo 领取状态按钮字体颜色*/
    }
  }
  .RechargeRebate_TopBox_Article {
    ...
    width: 660px;
    height: 320px;
    background: url('@/pages/summerParty2024/assets/mk2_3.png') no-repeat left top/100% 100%; /*todo 首充充值文本背景 图片或者手写渐变*/
    border-radius: 30px;
    .articleTitle {
      position: relative;
      margin: 0 auto 20px;
