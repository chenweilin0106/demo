<template>
  <div class="RechargeRebate">
    <FirstDouble v-bind="firstDoubleData" @recharge="recharge(hiddenFn)" @updateFirstDoubleData="updateFirstDoubleData" />
    ...
    >
      ...
      <RechargeTasksList v-show="RechargeGiftListMark == '累计充值有礼'" :RechargeGiftListData="recharge_status" :transitionClass="transitionClass" @recharge="recharge(hiddenFn)" @updateRechargeData="updateRechargeData" />
      <RechargeTasksList v-show="RechargeGiftListMark == '连续充值有礼'" :RechargeGiftListData="accruing_days_list" :transitionClass="transitionClass" @recharge="recharge(hiddenFn)" @updateRechargeData="updateRechargeData" />
    ...
    return {
      accruing_days_list: {
        ...
        3000: {
          amount_cond: 3000,
          awards: [
            { icon: 'zjs_60_60.png', id: 142, name: '紫晶石', nums: 1000, text: '1000', type: 'tool' },
...
.RechargeRebate {
  ...
  .RechargeRebate_BottomBox {
    width: 726px; /*todo 宽度 去看图片原始宽度，不要看蓝湖*/
    margin-top: 57px; /*todo 距离礼盒距离*/
    border-image-slice: 120 0 321 0 fill; /*todo 顶部距离是一般tab高度 底部距离一般和outBox一致*/
    border-image-width: 120px 0 321px 0; /*todo 顶部距离是一般tab高度 底部距离一般和outBox一致*/
    border-image-source: url('@/pages/summerParty2024/assets/zs_3.png'); /*todo 连续充值有礼背景*/
    .RechargeRebate_BottomBox_Tab {
      ...
      align-items: center;
      width: 100%;
      height: 111px; /*todo tab高度*/
      .resetIcon {
        top: 2px; /*todo 重置图标 距离tab top*/
        left: 6px; /*todo 重置图标 距离tab left*/
        width: 124px;
        height: 42px;
        background: url('@/pages/summerParty2024/assets/mk2_1.png') no-repeat left top/100% 100%; /*todo 图片名 可能要改*/
      }
      .update-1 {
        position: absolute;
        bottom: -2px; /*todo 重置图标 距离tab bottom*/
        left: 279px; /*todo 重置图标 距离tab left*/
        width: 96px;
        height: 128px;
        background: url('@/pages/summerParty2024/assets/mk2_5.png') no-repeat left top/100% 100%; /*todo 图片名 可能要改*/
      }
      .tab {
        position: relative;
        flex: 1;
  ...
  .active-tab-bg {
    border-image-source: url('@/pages/summerParty2024/assets/zs_4.png'); /*todo 连续充值有礼边框背景图片名*/
  }
}
</style>
