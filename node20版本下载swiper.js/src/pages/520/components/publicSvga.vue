<template>
  <div :id="svgaName" class="PublicSvga flex align-center justify-center overflow-hidden">
    <!-- 用户头像 -->
 ...
 *
 * @example
 * <PublicSvga svgaName="svagName" :userAvatar="$store.state.user_icon" />
 */
import SVGA from 'svgaplayerweb'

export default {
  name: 'PublicSvga',
  props: ['userAvatar', 'svgaName', 'loops'],
  computed: {
    svgaId() {
      return `${this.svgaName}_${Date.now()}_${Math.floor(Math.random() * 10000)}`
    }
  },
  watch: {
    svgaName: {
  ...
  methods: {
    ...
    loadSvga() {
      this.$nextTick(() => {
        this.clearSvga()
        this.svga_data = this.svgaFn(this.svgaName, this.svgaId, this.loops)
        // this.svga_data.onFinished((res) => {
        //   this.$emit('onFinished')
        // })
      })
    },
    svgaFn(svga_name, id, loops = 0) {
      const svga_play = new SVGA.Player(`#${id}`)
      const svga_parser = new SVGA.Parser(`#${id}`)
      svga_play.loops = loops
      svga_parser.load(`https://imagevo.dandanvoice.com/activity/weekly/svga/${svga_name}.svga`, (videoItem) => {
