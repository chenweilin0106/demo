<template>
  <div class="imgCompo">
    ...
    <template v-if="imgPattern === 1">
      <img class="picImg" :src="imgPath" @error.once="imgError"/>
    </template>
    <!-- svga -->
    <template v-else-if="imgPattern === 2">
      <!-- 用户头像 -->
      <div class="micAvatar" v-if="userAvatar">
        <img :src="userAvatar" @error.once="imgAtError"/>
      </div>
      <!-- svga容器 -->
...
export default {
  name: 'PublicImg',
  props: ['imgName', 'userAvatar', 'loop', 'needPause'],
  components: {},
  data() {
    return {
  ...
  methods: {
    ...
    loadSvga() {
      this.$nextTick(() => {
        this.clearSvga()
        this.svga_data = this.svgaFn(this.imgPath, this.svgaId)
      })
    },
    /**
     * 销毁svga
