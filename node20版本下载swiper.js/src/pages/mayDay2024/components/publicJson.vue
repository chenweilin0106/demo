<template>
  <div class="PublicJson position-relative w-100 h-100 flex align-center justify-center">
    <!-- 用户头像 -->
    <div v-if="userAvatar" class="micAvatar_img position-absolute flex align-center justify-center w-100 h-100">
      <img :src="userAvatar" class="w-100 h-100 fit-cover radius-50" @error.once="loadAvatarError" />
    </div>
    <!-- json容器 -->
    <div v-if="jsonName" class="json_box flex align-center justify-center w-100 h-100" :class="'PublicJson_' + jsonName" :id="JsonId" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    loadJson() {
      this.$nextTick(() => {
        this.destroyJson()
        this.PublicJsonObj = jsonFn(this.jsonName, `#${this.JsonId}`)
        // if (!this.isLoop) {
