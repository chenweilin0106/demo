<template>
  <div class="PublicSvga w-100 h-100 position-relative flex align-center justify-center overflow-hidden">
    <!-- 用户头像 -->
    <div v-if="userAvatar" class="micAvatar_img position-absolute flex align-center justify-center w-100 h-100">
      <img :src="userAvatar" class="w-100 h-100 radius-50 fit-cover" @error.once="loadAvatarError" />
    </div>
    <!-- svga容器 -->
    <div v-if="svgaName" class="svga_box w-100 h-100" :class="'PublicSvga_' + svgaName" :id="svgaId" />
  </div>
</template>
