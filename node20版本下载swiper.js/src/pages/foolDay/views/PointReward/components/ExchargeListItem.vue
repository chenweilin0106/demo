    ...
    <div class="inner">
      <div class="left-content flex flex-column">
        ...
        <div class="icon-content">
          ...
          <!-- json容器 -->
          <template v-else>
            <PublicJson v-if="exchargeItem.goods_type === 'title'" class="title" jsonName="ch_hysk" />
            <PublicSvga v-else class="mic" svgaName="ts_hql" :userAvatar="$store.state.user_icon" />
          </template>
        </div>
        <!-- 道具名称 -->
  ...
  .inner {
    ...
    .left-content {
      ...
      .icon-content {
        ...
        width: 100%;
        height: 125px;
        background: linear-gradient(180deg, #f3c0cd, #ffffff);
