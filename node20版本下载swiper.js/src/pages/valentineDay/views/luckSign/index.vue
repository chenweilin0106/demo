<template>
  <div class="dailyFortune" @click="pover_flag = null">
        ...
        <div class="fortune" v-if="sign_type != 0">
          ...
          <div class="big_good" v-if="sign_type == 3">
            ...
            <div class="fortune_detail">
              ...
              <div class="fortune_detail_bottom">
                7.5折（50%）、5折（30%）、2.5折（10%）免费（10%）
              </div>
            </div>
            <div class="overtime">
          ...
          <div class="middle_good" v-if="sign_type == 2">
            ...
            <div class="fortune_detail">
              <div class="fortune_detail_top">
                每消耗10<img
                  src="../../assets/thb_120_120.png"
                  alt=""
                />可自动获得1<img
                  src="../../assets/thb_120_120.png"
                  alt=""
                /><span class="special" :key="textKey"
                  >({{ pageData.sign_info.progress_ratio }})</span
                >
              </div>
              <div class="fortune_detail_bottom">
                本轮已获得桃花币奖励：{{ pageData.sign_info.get_coins }}
              </div>
            </div>
            <div class="overtime">
              本轮结束时间 <span class="special">{{ secTotime }}</span>
          ...
          <div class="small_good" v-if="sign_type == 1">
            ...
            <div class="fortune_detail">
              <div class="fortune_detail_top">
                ...
                累计消耗10<img src="../../assets/thb_120_120.png" alt="" /><span
                  class="blue"
                  >自动获得</span
                >1个<span class="yellow">心锁戒指</span
                ><span class="yellow"
                  >({{ pageData.sign_info.progress_ratio }})</span
                >
              </div>
    ...
    <out-box class="exchangeArea">
      <div
        class="question"
        @click="$refs.rewardDescription.showPopup = true"
      ></div>
      <div
        class="exchange_item"
      ...
      >
        <div class="fixed giftItem">
          <div class="jz999" v-if="index == 1">
            价值999<img :src="IconPath('zs_42_32.png')" alt="" />
          </div>
        ...
        <div class="random giftItem" @click.stop="pover_flag = index + 1">
          ...
          <div class="pover" v-if="pover_flag == index + 1">
            <div
              class="fixed giftItem"
              v-for="(item1, index1) in item.award2"
              :key="index1"
            >
              <div class="cornerMark" v-if="item1.type == 'costume'">
                主页特效
              </div>
              <div class="giftItem_top">
        ...
        <!-- 兑换按钮 -->
        <div
          v-throttle="[exchange, 'click', 1000, item.goods_id, item.real_price]"
          class="exchange_btn flex-center"
        >
          {{ item.org_price
          }}<img src="../../assets/thb_120_120.png" alt="" />兑换
          <!-- 折扣浮标 -->
      ...
      <div class="ThemeBox_BottomText">
        <ul>
          <li>
            <span></span>玩家<i>每消费300钻石</i>可自动获得1{{ pointName
          ...
          <li>
            <span></span>{{ pointName
              ...
              alt=""
            />可兑换不同类型的礼包，每次兑换可随机获得一份附赠奖励。
          </li>
          <li>
            <span></span
            >玩家可抽取桃花签获得运势加成，加成的生效时间为1小时，倒计时结束后运势失效，玩家可开启新的运势
          </li>
          <li>
            <span></span>倒计时结束前也可消耗{{ pointName
              ...
              :src="IconPath(pointIcon)"
              alt=""
            />进行重新抽签，若抽到不同的运势，则原有的加成效果和时间即刻失效
          </li>
        </ul>
      </div>
    </out-box>
    ...
    init() {
      home({
        ...
        if (res.errno == 0) {
          ...
          if (
            res.data.sign_info.count_down &&
            res.data.sign_info.count_down >= 0
          ) {
