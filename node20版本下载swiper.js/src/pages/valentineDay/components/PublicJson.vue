<template>
  <div class="PublicJson flex-center">
    <!-- 用户头像 -->
    <div class="micAvatar_img flex-center" v-if="userAvatar">
      <img :src="userAvatar" />
    </div>
    <!-- json容器 -->
    <div
      v-if="jsonName"
      class="json_box flex-center"
...
export default {
  ...
  methods: {
    ...
    loadJson() {
      this.$nextTick(() => {
        this.destroyJson()
        this.PublicJsonObj = jsonFn(this.jsonName, `#${this.JsonId}`, this.loop)
        this.PublicJsonObj.addEventListener('DOMLoaded', () => {
          // console.log('json开始播放')
          this.$emit('DOMLoaded')
        })
        if (this.needPause) {
    ...
    destroyJson() {
      if (this.jsonName && this.PublicJsonObj) {
        ...
        this.PublicJsonObj.removeEventListener('DOMLoaded')
        this.PublicJsonObj.removeEventListener('complete')
