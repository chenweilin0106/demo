    ...
    >
      ...
      <div class="giftTab" v-show="tabflag == 2">
        <div
          ...
          :key="index"
          @click="changeGiftTab(item, key, index)"
        >
          <img :src="IconPath(gift_tab_pic(gift_tab_flag, key))" alt="" />
        </div>
      </div>
      <!-- 奖励预览 -->
...
export default {
  ...
  methods: {
    async changeGiftTab(item, key, index) {
      console.log(this.present_list, item, key, index)
      this.gift_tab_flag = key
      this.current_present = key
      ...
      if (res.errno == 0) {
        ...
        this.pageData.total_rank = res.data.total_rank
      } else {
    ...
    async changetab(i) {
      ...
      // 礼物榜
      if (i == 2) {
        this.gift_tab_flag = '94'
        this.current_present = '94'
        const res = await getPageData({
          type: 'common_top_present_tot_rank',
          mark: this.current_present
        })
    ...
    gift_tab_pic(gift_tab_flag, key) {
      if (key == 94) {
        return gift_tab_flag == key ? 'mk6_25.png' : 'mk6_25_1.png'
      }
      if (key == 34) {
        return gift_tab_flag == key ? 'mk6_26.png' : 'mk6_26_1.png'
      }
      if (key == 32) {
        return gift_tab_flag == key ? 'mk6_27.png' : 'mk6_27_1.png'
      }
    },
    async chooseday(key) {
