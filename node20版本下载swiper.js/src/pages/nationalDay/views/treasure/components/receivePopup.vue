<template>
  <PopupBox :title="'title_8.png'" :size="2" :isDown="true" @clickClose="clickClose">
    <div class="receiveTextDiv" v-if="receiveText" v-html="formattedReceiveText(receiveText)"></div>
...
export default {
  ...
  methods: {
     ...
     * 文案替换
     */
    formattedReceiveText() {
      const keywords = ['触发【好运卡】buff', '触发【翻倍卡】buff', '触发【增幅卡】buff', '触发【加速卡】buff']
      let result = this.receiveText
      keywords.forEach(keyword => {
        const regex = new RegExp(keyword, 'g')
        result = result.replace(regex, `<span style="color: #FFEC6C;">${keyword}</span>`)
      })
      return result
    }
  }
