<template>
  <PopupBox :title="'title_11.png'" :isDown="true" @clickClose="clickClose">
    ...
    <div class="main" ref="scrollDiv" @scroll="handleScroll">
      <div class="listItem" v-for="(item, index) in list" :key="index">
        <div class="text3" v-if="Number(item.is_double)">
          双倍buff触发<i>{{item.desc}}</i>
        </div>
        <div class="text1" v-else>
          获得<span v-if="item.is_rare == 1">稀有</span>奖励<i>{{item.desc}}</i>
        </div>
  ...
  methods: {
    getData() {
      if (this.page > this.lastPage) {
        ...
        getPageData({ type: 'sh_treasure_lt_log', mark: this.page }).then((res) => {
          if (res.errno == 0) {
            // console.log(res)
