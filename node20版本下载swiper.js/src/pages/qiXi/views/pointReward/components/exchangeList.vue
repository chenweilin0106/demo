<template>
  <OutBox class="exchangeList" title="title_1.png" :bottomGap="false">
    <!-- 我的道具 -->
...
@mixin textMixin() { /*todo 奖励名称， 我的道具文本背景色*/
  background-color: #4F72D5; // 背景色
  color: #fff; // 字体颜色
}
$cardBorderRadius: 12px; /*todo 卡片圆角,卡片奖励左上和左下圆角,锁定阴影左上角圆角*/
.exchangeList {
  margin-top: 93px; /*todo 兑换列表上方margin*/
  padding-top: 0; /*todo 兑换列表上方padding*/
  // 顶部道具数量
  .my-goods {
    display: flex;
    align-items: center;
    ...
    @include textMixin;
    > img {
      width: 46px; /*todo 我的道具图标宽度*/
      height: 45px; /*todo 我的道具图标高度*/
    }
  }
  .exchange-item {
    position: relative;
    display: flex;
    margin: 0 auto 25px; /*todo 卡片下方间距*/
    width: 654px; /*todo 卡片宽度 不需要加上外圈描边宽度*/
    ...
    .left-content {
      height: 100%;
      width: 240px;
      //background: url('@/pages/qiXi/assets/') no-repeat left top/100% 100%; // 背景
      .icon-content {
        height: 124px;
        background: linear-gradient(180deg, #FFF8D5, #FFFFFF); /*todo 兑换列表中道具图标背景色*/
