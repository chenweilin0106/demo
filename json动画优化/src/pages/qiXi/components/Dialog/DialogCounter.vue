<template>
  <div class="DialogCounter">
    ...
    <div class="billing" v-if="showBilling">
      ...
      <!-- 兑换按钮 -->
      <div class="theme_botton_undid button" @click="clickSettlement">
        {{ buttonText }}
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    // 计数器加号按钮图标
    addIcon() {
      const { max_times, exchange_times } = this.goods_detail
      if (!+max_times) {
        return this.inputNum < this.unlimite_num ? 'pop_2.png' : 'pop_2_1.png'
      } else {
        return +max_times > +exchange_times + this.inputNum
          ? 'pop_2.png'
          : 'pop_2_1.png'
      }
    },
