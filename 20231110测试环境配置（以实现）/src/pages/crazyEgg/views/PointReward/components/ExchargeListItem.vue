  ...
  <div class="ExchargeListItem">
    <div class="ExchargeListItem_Left flex_vertical">
      ...
      <div class="ExchargeItem_Icon flex_across">
        <img
          ...
          :class="{ title_class: exchargeItem.goods_icon == 'ch_lzn.png' }"
        ...
        <div v-else class="pointRewardLottie flex_across">
          ...
          <public-svga
            class="mic_animation"
            v-if="exchargeItem.goods_type === 'mic'"
            svgaName="ts_tlzy"
      ...
      <!-- 道具名称 -->
      <div class="ExchargeItem_Name flex_across">
    ...
    <div class="ExchargeListItem_Center">
      <p
        ...
        "
      >
        消耗<span class="">{{ exchargeItem.price }}个{{ pointName }}</span
        >兑换 第二个<span>半价</span>
      </p>
      <p v-else>
        消耗<span class="">{{ exchargeItem.price }}个{{ pointName }}</span
...
.ExchargeListItem {
  ...
  padding: 3px;
  //border-radius: 15px;
  background: url('@/pages/crazyEgg/assets/bg_2.png') no-repeat left top/100%
    auto;
  // 图片样式
