  ...
  <div class="TaskListItem">
    <div class="TaskListItem_Main">
      ...
      <div v-if="isTwoPresent" class="TaskListItem_TextBox flex_across">
        {{ taskItem.task_name }}
        <span v-if="taskItem.task_name == '每充值10元'"
          >系统自动派发，无需手动领取</span
        >
        <span class="speical_color flex_across num_box" v-else
      ...
      <div class="TaskListItem_IconBox theme_item">
        ...
        <div class="img-bottom TaskListItem_BottomText">
          <span class="flex_across"
            >{{ pointName }}+{{ taskItem.award_nums }}</span
          >
        </div>
      </div>
      <!-- 闯关币 -->
      <div
        class="TaskListItem_IconBox theme_item"
        v-if="isTwoPresent && taskItem.piece"
      >
        <div class="img-top TaskListItem_TopIcon">
