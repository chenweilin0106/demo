<template>
  <div class="returnCourteous-page">
    ...
    <div class="depositRebate-bottom-body">
      <czItem v-for="(value, key) in recharge_status" :key="key" :itemValue="value" @getAward="clickGetAward"
        @toRecharge="$emit('toRecharge')"></czItem>
      <!-- 最后一项奖励 -->
      <div class="last-item-box">
        <div class="bottom-item-left">
          <p>
            <span>累计</span>充值满<span>{{ recharge_status[5000]?.condition }}元 ({{
              recharge_status[5000]?.recharge_nums
            }}/{{ recharge_status[5000]?.condition }})</span>
          </p>
          <div class="gift-box">
            <template v-for="item in recharge_status[5000]?.awards">
              <div class="gift-item" :key="item.name" v-if="!(item.name === '威望值')">
                <div class="img-box">
                  <div class="img-top">
                    <img v-if="item.type !== 'title'" v-lazy="require(`../../../../assets/${item.icon}`)" alt="" />
                    <div v-else class="rechargeRebate_lottie"></div>
                  </div>
        ...
        <div class="bottom-item-button" @click="clickGetAward(
...
import ic_ch_rsyj from '../../../../assets/lottie/ic_ch_rsyj/data.json'
export default {
  // 累计充值返利
  ...
  mounted() {
    svgFun(0, ic_ch_rsyj, 'ic_ch_rsyj', '.rechargeRebate_lottie')
  },
  computed: {},
  watch: {},
  created() { },
...
.returnCourteous-page {
  .depositRebate-bottom-body {
    ...
    .last-item-box {
      width: 332px;
      background: linear-gradient(-90deg, #FF90FE, #A399FF);
      ...
      .bottom-item-button {
        ...
        line-height: 32px;
        border-radius: 32px;
        background: linear-gradient(0deg, #FF561F, #FFB67D);
      ...
      .bottom-item-button-did {
        background: linear-gradient(0deg, #8A8A8A, #D4D4D4) !important;
        color: #fff !important;
      }

      // 去完成
      .bottom-item-button-unDid {
        background: linear-gradient(180deg, #FFFEE9, #FFEB90) !important;
        color: #FB2505 !important;
      }
    }
  }
