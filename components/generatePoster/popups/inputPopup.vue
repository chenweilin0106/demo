<template>
  <PopupBox title="title_17.png" @clickClose="clickClose">
    ...
    <div class="loveStoryButtons flex align-center justify-center">
      <PublicButton hasRight="0" class="cancel" @click="clickClose">我再想想</PublicButton>
      <PublicButton hasRight="1" class="confirm" @click="submitLoveStory">确认提交</PublicButton>
    </div>
  </PopupBox>
</template>
...
export default {
  ...
  methods: {
    ...
    async submitLoveStory() {
      ...
      const res = await getPageData({ type: 'lovename_award_rev', 'mark[goods_id]': 4, 'mark[action_type]': 1, 'mark[id]': 0, 'mark[love_words]': this.loveStory })
    ...
    // 输入自定义情话
    inputLoveStory(e) {
      if (e.target.value.indexOf(' ') >= 0) this.uid = e.target.value.replace(/\s+/g, '') // 删除空格及回车
      this.errTip = '' // 清除错误提示
    },
    clickClose() {
    ...
    // 随机抽取情话
    randomLoveStory() {
      this.loveStory = this.config[Math.floor(Math.random() * this.config.length)]
      this.errTip = '' // 清除错误提示
    }
  }
}
...
.loveStoryInput {
  ...
  height: 138px;
  background: #fefaef;
  border: 4px solid #FFBCDC; /*todo 输入框边框背景色*/
  border-radius: 41px;
  > textarea {
    width: 478px;
    height: 100%;
  ...
  .refresh {
    flex: 1;
    height: 100%;
    background: #FFC8E2; /*todo 刷新按钮背景色*/
