 ...
 *'icon'=>'ts_hsyh', //图片
 *},
 * @param {object} moreList - 更多展示字段
 * @param {array} nameList - 更多需要展示name值的字段
 */
export function toShowText(item, moreList = {}, nameList = []) {
  let typeNameList = {
    ring: '戒指',
    mic: '头饰',
    car: '座驾',
  ...
  let needNameList = ['rune']
  if (nameList) {
    needNameList = [...needNameList, ...nameList]
  }
  ...
  if (item.nums <= 0) {
    if (item.type == 'title') {
      ...
      return item.name
    } else if (item.type == 'virtual_avatar_components_suite') {
      return '全套时装'
    }
  } else {
    if (needNameList.includes(item.type)) {
